---
title: 'PHP'
description: 'Implementação completa da API FlowPay em PHP com exemplos práticos'
icon: 'php'
---

## Implementação PHP

Guia completo para integrar a API FlowPay em suas aplicações PHP usando cURL ou Guzzle.
...
class FlowPayAPI {
    private const BASE_URL = 'https://www.flowapplications.com.br/api/v1';
    private $apiToken;
    private $httpClient;
...
try {
    ...
    if ($resultado['success'] && isset($resultado['charge'])) {
        ...
        echo "🔲 QR Code: {$charge['qrCodeImage']}\n";
        echo "🔲 QR Code: {$charge['qrCodeImage']}\n";
        echo "📋 BR Code: {$charge['brCode']}\n";
    } else {
...
class FlowPayService {
    private $client;
    private $baseUrl = 'https://www.flowapplications.com.br/api/v1';
    private $apiToken;
