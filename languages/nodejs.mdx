---
title: 'Node.js'
description: 'Implementação completa da API FlowPay em Node.js com exemplos práticos'
icon: 'square-code'
---

## Implementação Node.js

Guia completo para integrar a API FlowPay em suas aplicações Node.js usando axios ou fetch nativo.
...
async function criarPagamento(correlationID, valor, comentario = null) {
  ...
  try {
    ...
    if (success && charge) {
      ...
      console.log('📱 Link de pagamento:', charge.paymentLinkUrl);
...
export class PaymentService {
  private readonly baseURL = 'https://flowapplications.com.br/api/v1';
...
async function verificarPagamento(correlationID) {
  try {
    ...
    if (success && charge) {
      console.log(`📊 Status do pagamento: ${charge.status}`);
      ...
      throw new Error(error || 'Pagamento não encontrado');
    }
  } catch (error) {
    ...
    if (response.data.success) {
      res.json({
        ...
        pagamento: {
          ...
          link: response.data.charge.paymentLinkUrl,
...
FLOWPAY_BASE_URL=https://flowapplications.com.br/api/v1
```
